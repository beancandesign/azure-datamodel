...
from azure_datamodel import Model, Foundation, Blade, Nacelle, Rotor, Hub, Tower
from azure_datamodel.fea.elements import (
    Tube,
    Cuboid,
    Cone,
    ElementSet,
...
from azure_datamodel.fea.nodes import Node
from azure_datamodel.geometry.vectors import Vector3
...
def model():
    ...
    foundation_elements[tp + 1] = Tube(
        id=tp + 1,
        nodes=[
            Node(x=0, y=0, z=TOP + TP_HEIGHT / 5),
            Node(x=0, y=0, z=TOP + TP_HEIGHT),
        ],
        ...
        ConicalTube(
            nodes=[
                Node(
                    x=0,
                    y=0,
                    z=TOP + TP_HEIGHT + TOWER_HEIGHT * idx / NUM_TOWER_ELEMENTS,
                ),
                Node(
                    x=0,
                    y=0,
